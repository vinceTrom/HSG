...
public abstract class Character extends Renderable{
	...
	protected void createAnims(Context ctx, String fileName){
		...
		boolean typePlayer = elem.getElementForKey("player")!=null;
		if(typePlayer){
			...
			MMXMLElements elems = elem.getElementForKey("soldier").getElementForKey("animations").getElementsForKey("animation");
			for(int i=0;i<elems.size();i++){
				String anim_name = elems.get(i).getAttributes().get("name");
				GLAnim anim = new GLAnim("soldier/"+anim_name, true, this);
				GLUtils.createAnim(ctx, anim, elems.get(i));
				_sprites.add(anim);
			}
		}
